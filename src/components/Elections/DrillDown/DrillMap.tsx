...
export default function DrillMap({
            ...
            mouseover(e) {
              ...
              if (child) {
                ...
                const lines = child.candidates
                  ...
                  .map(
                    (c) =>
                      `${c.name}: ${total > 0 ? ((c.votes / total) * 100).toFixed(1) : 0}%`,
                  )
                  .join("<br/>");
  ...
  const legendItems = data.candidates
    .filter((c) => c.votes > 0)
    .slice(0, 5)
    .map((c) => ({
      name: c.name,
  ...
  return (
    ...
    >
      ...
      <Box ref={mapRef} h="calc(100vh - 280px)" minH="440px" w="full" borderRadius="xl" overflow="hidden" />
      ...
      {legendItems.length > 0 && (
        ...
        >
          ...
          {legendItems.map((item) => (
            <Flex key={item.name} align="center" gap={2} mb={1} _last={{ mb: levelItems ? 0 : 0 }}>
