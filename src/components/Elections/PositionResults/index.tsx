...
/** Map is only available for levels that have matchable geojson files */
const MAP_LEVELS = new Set(["COUNTY", "CONSTITUENCY"])
...
function PositionCard({
  ...
  const canExplore =
    hasVotes &&
    ["NATIONAL", "COUNTY", "CONSTITUENCY", "WARD", "STATION"].includes(
  ...
  const exploreProps = canExplore
    ...
    : undefined;
  const hasMap = hasVotes && MAP_LEVELS.has(position.aggregationLevel);
  const [view, setView] = useState<"map" | "details">(
    hasMap ? "map" : "details",
  );
          ...
          <HStack gap={2}>
            ...
            {hasMap && (
              <HStack gap={1} p={1} bg="gray.100" borderRadius="full">
                {(["map", "details"] as const).map((v) => (
                  <HStack
                    key={v}
                    as="button"
                    aria-pressed={view === v}
                    aria-label={v === "map" ? "Map view" : "Details view"}
                    gap={1.5}
                    px={3}
                    py={1.5}
                    borderRadius="full"
      ...
      {position.streamStats.rejectedVotes > 0 && (
        ...
        >
          <Text fontSize="xs" color="#92400e">
            {position.streamStats.rejectedVotes.toLocaleString()} rejected votes
            not counted above
          </Text>
        </Box>
      )}
