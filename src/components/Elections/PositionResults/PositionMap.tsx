...
export default function PositionMap({
  ...
  const candidateColorMap = (() => {
    ...
    sorted.forEach(([id], i) => ranked.set(id, CANDIDATE_PALETTE[i % CANDIDATE_PALETTE.length]))
    return ranked
  })()
    ...
    async function init() {
      ...
      const geoLayer = L.geoJSON(geojson, {
        style: (feature) => {
          ...
          if (!entity) return { fillColor: "#d1d5db", fillOpacity: 0.3, color: "#fff", weight: 1 }

          const leader = entity.candidates[0]
          const total = entity.candidates.reduce((s, c) => s + c.streamVotes, 0)
          const pct = total > 0 ? (leader.streamVotes / total) * 100 : 0
          const fill = leader ? (candidateColorMap.get(leader.id) ?? "#d1d5db") : "#d1d5db"
          return {
            fillColor: fill,
          ...
          layer.on({
            mouseover(e) {
              ...
              const fill = entity?.candidates[0]
                ...
                : lc.color
              l.setStyle({ weight: 2.5, color: fill, fillOpacity: Math.min(1, (l.options.fillOpacity ?? 0.4) + 0.15) })
              if (entity) {
                const total = entity.candidates.reduce((s, c) => s + c.streamVotes, 0)
                const lines = entity.candidates.slice(0, 3)
                  .map((c) => `${c.name}: ${total > 0 ? ((c.streamVotes / total) * 100).toFixed(1) : 0}%`)
                  .join("<br/>")
      ...
      const matchedGroup = matchedLayers.length > 0 ? L.featureGroup(matchedLayers) : geoLayer
  ...
  const { legendItems, streamGrandTotal } = (() => {
    ...
    for (const entity of entities)
      for (const c of entity.candidates)
        totals.set(c.id, { name: c.name, party: c.party ?? "", votes: (totals.get(c.id)?.votes ?? 0) + c.streamVotes })
  ...
  return (
    <Box position="relative" overflow="hidden" borderTopWidth="1px" borderBottomWidth="1px" borderColor="gray.200" px={4} pb={4} pt={3}>
      <style>{`.drill-map-tip { font-size: 12px; line-height: 1.5; }`}</style>
      <Box ref={mapRef} h="calc(100vh - 320px)" minH="420px" w="full" borderRadius="xl" overflow="hidden" />

      {/* ── Legend ── */}
      {legendItems.length > 0 && (
        <Box
          position="absolute" top={7} left={8}
          bg="white" borderRadius="lg" boxShadow="md"
          px={3} py={2.5} zIndex={1000}
          borderWidth="1px" borderColor="gray.100"
          minW="180px"
        >
          {/* Stream-aggregate title */}
          <Text fontSize="11px" fontWeight="700" color="gray.600" mb={2}>
            {levelLabel}: Stream Aggregate — {streamGrandTotal.toLocaleString()} votes
          </Text>

          {/* Color key — stream aggregate */}
          {legendItems.map((item) => (
            <Flex key={item.name} align="center" gap={2} mb={1} _last={{ mb: 0 }}>
              ...
              <Text fontSize="11px" fontWeight="600" color="gray.700" flex={1} lineClamp={1}>
                {item.name}{item.party ? <Text as="span" fontWeight="400" color="gray.400"> ({item.party})</Text> : null}
              </Text>
              <Text fontSize="11px" color="gray.500">{item.votes}</Text>
          ...
          {levelItems && (
            <>
              <Box borderTopWidth="1px" borderColor="gray.100" mt={2} mb={1.5} />
              <Flex
                align="center"
              ...
              >
                <Text fontSize="10px" fontWeight="700" color="gray.500" lineClamp={1}>
                  {formRef.form} · {formRef.label}
                </Text>
                <Text fontSize="10px" color="gray.400" ml={2}>{resultsOpen ? "▲" : "▼"}</Text>
              </Flex>
              {resultsOpen && (
                <>
                  {levelItems.rows.map((item) => (
                    <Flex key={item.name} align="center" gap={1.5} mb={1}>
                      <Box w={2.5} h={2.5} borderRadius="sm" flexShrink={0} bg={item.color} />
                      <Text fontSize="11px" color="gray.700" flex={1} lineClamp={1}>
                        {item.name}{item.party ? <Text as="span" color="gray.400"> ({item.party})</Text> : null}
                      </Text>
                  ...
                  {levelItems.rejectedVotes > 0 && (
                    <Flex align="center" gap={1.5} mt={0.5}>
                      <Box w={2.5} h={2.5} borderRadius="sm" flexShrink={0} bg="red.300" />
                      <Text fontSize="11px" color="red.600" flex={1}>Rejected</Text>
                      <Text fontSize="11px" color="gray.500" ml={1}>{levelItems.rejectedVotes.toLocaleString()}</Text>
                      <Text fontSize="11px" fontWeight="700" color="red.700" ml={1}>
                        {levelItems.totalVotes > 0 ? (levelItems.rejectedVotes / levelItems.totalVotes * 100).toFixed(1) : "0.0"}%
                      </Text>
                    </Flex>
                  )}
                </>
      ...
      <Box
        position="absolute" bottom={8} right={8} fontSize="10px" color="gray.400"
