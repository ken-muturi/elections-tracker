...
export default async function EnterResultsPage() {
      ...
      <VStack gap={6} alignItems="stretch" w="full">
        <Flex justify="space-between" align="center" flexWrap="wrap" gap={3}>
          <VStack alignItems="flex-start" gap={0.5}>
            ...
            <Text fontSize="sm" color="gray.500">Search for a stream and enter vote counts for each position</Text>
          </VStack>

          <HStack
            gap={2.5} px={3} py={2} borderRadius="xl"
            bg="white" borderWidth="1px" borderColor="gray.100"
          ...
          >
            ...
            <Box>
              ...
              <Badge size="xs" colorPalette="purple" variant="subtle" fontSize="9px" fontWeight="700">
                {user?.role ?? "Admin"}
              </Badge>
            </Box>
          </HStack>
        ...
        <EnterResultsClient electionData={JSON.parse(JSON.stringify(electionData))} isAdmin />
      </VStack>
    )
  }
  ...
  // Group by election
  const electionMap = new Map<string, {
  ...
  const electionData = await Promise.all(
    elections.map(async (e) => {
      ...
      const streamResultsMap: Record<string, Awaited<ReturnType<typeof getStreamResultsForStream>>> = {}
      for (const a of e.streams) {
        streamResultsMap[a.streamId] = await getStreamResultsForStream(a.streamId)
      }
      return {
        election: e.election,
        streams: e.streams,
  ...
  if (electionData.length === 0) {
    return (
      <VStack gap={6} alignItems="center" justifyContent="center" minH="60vh">
        ...
        <VStack gap={1}>
          <Heading fontSize="xl" fontWeight="700" color="gray.700">No Active Assignments</Heading>
    ...
    <VStack gap={6} alignItems="stretch" w="full">
      <Flex justify="space-between" align="center" flexWrap="wrap" gap={3}>
        <VStack alignItems="flex-start" gap={0.5}>
          ...
          <Text fontSize="sm" color="gray.500">Select a stream and enter vote counts for each position</Text>
        </VStack>
        ...
        >
          ...
          <Box>
            ...
            <Badge size="xs" colorPalette="blue" variant="subtle" fontSize="9px" fontWeight="700">
              {user?.role ?? "Agent"}
            </Badge>
          </Box>
        </HStack>
      ...
      <EnterResultsClient electionData={JSON.parse(JSON.stringify(electionData))} />
    </VStack>
  )
}
