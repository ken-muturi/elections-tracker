    ...
    <VStack gap={6} alignItems="stretch" w="full">
        ...
        <HStack justify="space-between" align="flex-end" w="full" flexWrap="wrap" gap={3}>
          ...
          <HStack gap={2}>
            ...
            <Link href={`/elections/${id}/results`}>
              <HStack gap={2} px={4} py={2} bg="#eff9d1" color="#798217" borderRadius="lg"
                fontSize="sm" fontWeight="600" cursor="pointer" _hover={{ bg: "#e5f0a0" }} transition="background 0.15s">
                <FiBarChart2 />
            ...
            <Link href={`/elections/${id}/candidates`}>
              <HStack gap={2} px={4} py={2} bg="#0f172a" color="white" borderRadius="lg"
                fontSize="sm" fontWeight="600" cursor="pointer" _hover={{ bg: "#1e293b" }} transition="background 0.15s">
                <FiEdit />
      ...
      <Box>
        ...
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={4}>
          {election.positions.map((position) => {
            ...
            const resultData = results.find((r) => r.positionId === position.id)
            const allCandidates = resultData?.entities.flatMap((e) => e.candidates) ?? []
            const aggregated = new Map<string, { name: string; party: string | null; streamVotes: number }>()
            for (const c of allCandidates) {
              const existing = aggregated.get(c.id)
              aggregated.set(c.id, { name: c.name, party: c.party, streamVotes: (existing?.streamVotes ?? 0) + c.streamVotes })
            }
            const topCandidate = [...aggregated.values()].sort((a, b) => b.streamVotes - a.streamVotes)[0]
            ...
            return (
              ...
              >
                <HStack justify="space-between" mb={3}>
                  <Flex w={9} h={9} borderRadius="lg" bg={levelColor.bg} align="center" justify="center">
                    <MdHowToVote fontSize="1rem" color={levelColor.color} />
                  </Flex>
                  <Badge
                    px={2.5} py={1} borderRadius="full"
                    bg={levelColor.bg} color={levelColor.color}
                ...
                <Text fontWeight="700" fontSize="sm" color="gray.900" mb={0.5}>
                  {position.title}
                </Text>
                <Text fontSize="xs" color="gray.500" mb={3}>{meta?.description ?? position.description ?? ""}</Text>
                ...
                {topCandidate && topCandidate.streamVotes > 0 && (
                  <Box mt={3} p={2.5} bg="#f8fafc" borderRadius="lg">
                    <Text fontSize="9px" color="gray.400" fontWeight="600" textTransform="uppercase" letterSpacing="wide" mb={0.5}>
                      Leading
                    </Text>
                    <HStack justify="space-between">
                    ...
                    {topCandidate.party && (
                      <Text fontSize="9px" color="gray.400">{topCandidate.party}</Text>
                    )}
                  </Box>
                )}
              </Box>
      ...
      <Box>
        ...
        <SimpleGrid columns={{ base: 2, md: 3 }} gap={4}>
          {results.map((r) => (
            <Box key={r.positionId} bg="white" p={4} borderRadius="xl" borderWidth="1px" borderColor="gray.100"
              boxShadow="0 1px 3px 0 rgba(0,0,0,0.06)">
              ...
              <Text fontSize="xs" color="gray.400">
                {r.streamStats.totalReported} streams · {r.levelValidations} validations
              </Text>
            </Box>
          ))}
        </SimpleGrid>
