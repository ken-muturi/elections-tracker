...
export default function ElectionHierarchyManager({
  ...
  const emptyTopForm = {
    countyName: "", constituencyId: "", wardId: "",
    wardName: "", wardCode: "",
    stationName: "", stationCode: "", voters: "",
  }
  ...
  const patchStream = (idx: number, patch: Partial<InlineStream>) =>
    setAddTopStreams((prev) => prev.map((s, i) => i === idx ? { ...s, ...patch } : s))

  const stationReady = addTopForm.stationName.trim() && addTopForm.stationCode.trim()

  const handleAddTopStation = () => {
    const f = addTopForm
    if (!f.constituencyId) { setAddTopError("Select a constituency"); return }
    if (!f.wardId) { setAddTopError("Select or create a ward"); return }
    if (f.wardId === "__new__" && (!f.wardName.trim() || !f.wardCode.trim())) {
      setAddTopError("Ward name and code are required"); return
    }
    if (!f.stationName.trim() || !f.stationCode.trim()) {
      setAddTopError("Station name and code are required"); return
    }
    for (let i = 0; i < addTopStreams.length; i++) {
      if (!addTopStreams[i].name.trim() || !addTopStreams[i].code.trim()) {
        setAddTopError(`Stream ${i + 1}: name and code are required`); return
      }
    }
    setAddTopError("")
  ...
  const handleDeleteWard = (wardId: string) => {
    startTransition(async () => {
      try {
        ...
        toaster.error({ title: "Delete failed", description: e instanceof Error ? e.message : "Unknown" })
      }
    })
  }

  // ── Add Polling Station ───────────────────────────────────────────────────
  const [addingStationToWard, setAddingStationToWard] = useState<string | null>(null)
  ...
  const handleAddStation = (wardId: string, values: Record<string, string>) => {
    if (!values.name || !values.code) { setAddStationError("Name and code are required"); return }
    setAddStationError("")
    ...
    startTransition(async () => {
      try {
        ...
        setWards((prev) =>
          prev.map((w) =>
            w.id === wardId
              ? {
                  ...w,
                  pollingStations: [...w.pollingStations, newStation].sort((a, b) =>
        ...
        setAddStationError(e instanceof Error ? e.message : "Failed to add station")
      } finally {
    ...
    startTransition(async () => {
      try {
        ...
        setWards((prev) =>
          prev.map((w) =>
            w.id === wardId
              ? { ...w, pollingStations: w.pollingStations.filter((s) => s.id !== stationId) }
              : w
          )
        )
  ...
  // ── Add Stream ────────────────────────────────────────────────────────────
  const [addingStreamToStation, setAddingStreamToStation] = useState<string | null>(null)
  const [addStreamError, setAddStreamError] = useState("")
  const [addStreamPending, setAddStreamPending] = useState(false)

  const handleAddStream = (wardId: string, stationId: string, values: Record<string, string>) => {
    if (!values.name || !values.code) { setAddStreamError("Name and code are required"); return }
    setAddStreamError("")
    ...
    startTransition(async () => {
      try {
        ...
        setAddStreamError(e instanceof Error ? e.message : "Failed to add stream")
      } finally {
  ...
  const handleDeleteStream = (wardId: string, stationId: string, streamId: string) => {
    startTransition(async () => {
      try {
          ...
          prev.map((w) =>
            w.id === wardId
              ? {
                  ...
                  pollingStations: w.pollingStations.map((ps) =>
                    ps.id === stationId
                      ? { ...ps, streams: ps.streams.filter((s) => s.id !== streamId) }
                      : ps
                  ),
                }
              : w
  ...
  const uniqueCounties = [...new Set(constituencies.map((c) => c.county.name))].sort()
  const selStyle = (hasVal: boolean): React.CSSProperties => ({
    width: "100%", padding: "8px 12px", borderRadius: "8px",
    border: `1px solid ${hasVal ? "#d1d5db" : "#e5e7eb"}`,
    fontSize: "0.875rem", color: hasVal ? "#1f2937" : "#9ca3af",
    background: "white", cursor: "pointer", outline: "none",
  })

  // ── Group by county ────────────────────────────────────────────────────────
  type CountyGroup = { wards: WardData[] }
  const groups: Record<string, CountyGroup> = {}
  for (const ward of wards) {
    const c = ward.constituency.county.name
    if (!groups[c]) groups[c] = { wards: [] }
    groups[c].wards.push(ward)
  }
  ...
  return (
    <VStack gap={4} alignItems="stretch">
      ...
      <Box>
        {!showAddTopStation ? (
          <HStack
            ...
            borderRadius="lg" fontSize="sm" fontWeight="600" color="gray.600"
            cursor="pointer" _hover={{ bg: "gray.50", borderColor: "gray.300" }}
            transition="all 0.15s"
          ...
          >
            <Text fontSize="sm" fontWeight="600" color="gray.700" mb={4}>Add Polling Station</Text>
            <VStack gap={3} alignItems="stretch">

              {/* County + Constituency */}
              <HStack gap={3} flexWrap="wrap">
                <Box flex={1} minW="180px">
                  <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>County *</Text>
                  <select
                    style={selStyle(!!addTopForm.countyName)}
                    value={addTopForm.countyName}
                    onChange={(e) => patchTop({ countyName: e.target.value, constituencyId: "", wardId: "", wardName: "", wardCode: "" })}
                  >
                ...
                <Box flex={1} minW="180px">
                  <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Constituency *</Text>
                  <select
                    style={selStyle(!!addTopForm.constituencyId)}
                    value={addTopForm.constituencyId}
                    onChange={(e) => patchTop({ constituencyId: e.target.value, wardId: "", wardName: "", wardCode: "" })}
                  >
              ...
              {addTopForm.constituencyId && (
                <Box>
                  <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Ward *</Text>
                  <select
                    style={selStyle(!!addTopForm.wardId)}
                    value={addTopForm.wardId}
                    onChange={(e) => patchTop({ wardId: e.target.value, wardName: "", wardCode: "" })}
                  >
                    <option value="">Select ward…</option>
                    {wards
                      .filter((w) => w.constituencyId === addTopForm.constituencyId)
                      .sort((a, b) => a.name.localeCompare(b.name))
              ...
              {addTopForm.wardId === "__new__" && (
                <HStack gap={3} pl={3} borderLeftWidth="2px" borderLeftColor="#e0e7ff" flexWrap="wrap">
                  <Box flex={1} minW="150px">
                    <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Ward Name *</Text>
                    <Input size="sm" placeholder="e.g. Kinangop Ward 1" value={addTopForm.wardName}
                      onChange={(e) => patchTop({ wardName: e.target.value })}
                      borderRadius="lg" borderColor="gray.200" _focus={{ borderColor: "#0f172a", boxShadow: "none" }} fontSize="xs" />
                  </Box>
                  <Box flex={1} minW="120px">
                    <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Ward Code *</Text>
                    <Input size="sm" placeholder="e.g. WD-001" value={addTopForm.wardCode}
              ...
              {addTopForm.wardId && (addTopForm.wardId !== "__new__" || (addTopForm.wardName.trim() && addTopForm.wardCode.trim())) && (
                <HStack gap={3} pt={2} borderTopWidth="1px" borderTopColor="gray.100" flexWrap="wrap">
                  <Box flex={2} minW="200px">
                    <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Station Name *</Text>
                    <Input size="sm" placeholder="e.g. Kenyatta Primary School" value={addTopForm.stationName}
                      onChange={(e) => patchTop({ stationName: e.target.value })}
                      borderRadius="lg" borderColor="gray.200" _focus={{ borderColor: "#0f172a", boxShadow: "none" }} fontSize="xs" />
                  </Box>
                  <Box flex={1} minW="130px">
                    <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Station Code *</Text>
                    <Input size="sm" placeholder="e.g. PS-001" value={addTopForm.stationCode}
                      onChange={(e) => patchTop({ stationCode: e.target.value })}
                      borderRadius="lg" borderColor="gray.200" _focus={{ borderColor: "#0f172a", boxShadow: "none" }} fontSize="xs" />
                  </Box>
                  <Box flex={1} minW="130px">
                    <Text fontSize="xs" color="gray.500" fontWeight="600" mb={1.5}>Registered Voters</Text>
                    <Input size="sm" type="number" placeholder="Optional" value={addTopForm.voters}
              ...
              {stationReady && (
                <Box pt={2} borderTopWidth="1px" borderTopColor="gray.100">
                  <HStack justify="space-between" mb={2}>
                    <HStack gap={1.5}>
                      ...
                      <Text fontSize="xs" fontWeight="600" color="gray.600">
                        Streams {addTopStreams.length > 0 && <Text as="span" color="gray.400">({addTopStreams.length})</Text>}
                      </Text>
                    </HStack>
                    <HStack
                      as="button" gap={1} px={2} py={1} borderRadius="md"
                      fontSize="10px" fontWeight="600" color="gray.400"
                      cursor="pointer" _hover={{ color: "#16a34a", bg: "#dcfce7" }}
                      transition="all 0.15s"
                      onClick={() => setAddTopStreams((prev) => [...prev, { ...emptyStream }])}
                    >
                  ...
                  {addTopStreams.length === 0 && (
                    <Text fontSize="xs" color="gray.300">No streams yet — click &quot;Add Stream&quot; to define them now, or add later.</Text>
                  )}
                  <VStack gap={2} alignItems="stretch">
                    {addTopStreams.map((s, idx) => (
                      <HStack key={idx} gap={2} pl={3} borderLeftWidth="2px" borderLeftColor="#dcfce7" flexWrap="wrap">
                        <Box flex={2} minW="130px">
                          <Input size="sm" placeholder="Stream name e.g. Stream A" value={s.name}
                            onChange={(e) => patchStream(idx, { name: e.target.value })}
                            borderRadius="lg" borderColor="gray.200" _focus={{ borderColor: "#16a34a", boxShadow: "none" }} fontSize="xs" />
                        </Box>
                        <Box flex={1} minW="80px">
                          <Input size="sm" placeholder="Code e.g. A" value={s.code}
                            onChange={(e) => patchStream(idx, { code: e.target.value })}
                            borderRadius="lg" borderColor="gray.200" _focus={{ borderColor: "#16a34a", boxShadow: "none" }} fontSize="xs" />
                        </Box>
                        <Box flex={1} minW="80px">
                          <Input size="sm" type="number" placeholder="Voters" value={s.voters}
                            onChange={(e) => patchStream(idx, { voters: e.target.value })}
                            borderRadius="lg" borderColor="gray.200" _focus={{ borderColor: "#16a34a", boxShadow: "none" }} fontSize="xs" />
                        </Box>
                        <Flex
                          as="button" w={6} h={6} borderRadius="md" align="center" justify="center"
                          color="gray.300" cursor="pointer" flexShrink={0}
                          _hover={{ color: "#ef4444", bg: "#fef2f2" }} transition="all 0.15s"
                          onClick={() => setAddTopStreams((prev) => prev.filter((_, i) => i !== idx))}
                        >
                          <FiX fontSize="0.7rem" />
                        </Flex>
                      </HStack>
              ...
              {addTopError && <Text fontSize="xs" color="#dc2626">{addTopError}</Text>}

              <HStack gap={2}>
                <HStack
                  as="button" gap={1} px={3} py={1.5} borderRadius="lg"
                  bg="#0f172a" color="white" fontSize="xs" fontWeight="600"
                  cursor="pointer" _hover={{ bg: "#1e293b" }}
                ...
                >
                  {addTopPending ? <Spinner size="xs" /> : <FiCheck fontSize="0.7rem" />}
                  <Text>Save</Text>
                </HStack>
                <HStack
                  as="button" gap={1} px={3} py={1.5} borderRadius="lg"
                  ...
                  fontSize="xs" fontWeight="600" color="gray.500"
                  cursor="pointer" _hover={{ bg: "gray.50" }}
                  onClick={() => { setAddTopForm(emptyTopForm); setAddTopStreams([]); setShowAddTopStation(false); setAddTopError("") }}
                >
                  <FiX fontSize="0.7rem" />
                  <Text>Cancel</Text>
                </HStack>
      ...
      {wards.length === 0 && (
        <Box
          bg="white" p={10} borderRadius="xl" borderWidth="1px" borderColor="gray.100"
          textAlign="center"
        >
          <Flex w={16} h={16} borderRadius="2xl" bg="gray.50" align="center" justify="center" mx="auto" mb={4}>
            <FiMapPin fontSize="1.5rem" color="#9ca3af" />
          </Flex>
          <Text fontWeight="700" color="gray.700" mb={1}>No polling stations yet</Text>
      ...
      {Object.entries(groups)
        ...
        .map(([countyName, group]) => {
          const totalStations = group.wards.reduce((s, w) => s + w.pollingStations.length, 0)
          const totalStreams = group.wards.reduce(
            (s, w) => s + w.pollingStations.reduce((ss, ps) => ss + ps.streams.length, 0),
            0
          )
          ...
          return (
            ...
            >
              ...
              <HStack
                px={5} py={4} bg="#f8fafc"
                borderBottomWidth="1px" borderBottomColor="gray.100" gap={3}
              >
                <Flex
                  w={8} h={8} borderRadius="lg" bg="#e0e7ff"
                  align="center" justify="center" color="#4f46e5" fontSize="0.9rem"
                >
                  <MdOutlineLocationCity />
                </Flex>
                <Text fontWeight="700" color="gray.900" flex={1}>{countyName}</Text>
                <HStack gap={2}>
                  <Badge px={2} py={0.5} borderRadius="md" bg="#eff6ff" color="#1d4ed8" fontSize="xs" fontWeight="600">
                    {group.wards.length} ward{group.wards.length !== 1 ? "s" : ""}
                  </Badge>
                  <Badge px={2} py={0.5} borderRadius="md" bg="#f0fdf4" color="#166534" fontSize="xs" fontWeight="600">
                    {totalStations} station{totalStations !== 1 ? "s" : ""}
                  </Badge>
                  <Badge px={2} py={0.5} borderRadius="md" bg="#faf5ff" color="#7e22ce" fontSize="xs" fontWeight="600">
              ...
              <VStack gap={0} alignItems="stretch">
                {group.wards
                  ...
                  .map((ward, idx) => {
                    ...
                    const wardStreams = ward.pollingStations.reduce((s, ps) => s + ps.streams.length, 0)
                    const isAddingStation = addingStationToWard === ward.id
                    ...
                    return (
                      ...
                      >
                        ...
                        <HStack px={5} py={3} gap={3} bg="gray.50">
                          <Flex
                            as="button"
                            w={5} h={5} borderRadius="sm" bg="white" align="center" justify="center"
                            borderWidth="1px" borderColor="gray.200"
                          ...
                          >
                            {isOpen
                              ? <FiChevronDown fontSize="0.7rem" color="#6b7280" />
                              : <FiChevronRight fontSize="0.7rem" color="#6b7280" />
                            }
                          </Flex>
                          <Box w={1} h={4} bg="#c7d2fe" borderRadius="full" flexShrink={0} />
                          <Text fontSize="sm" fontWeight="600" color="gray.700" flex={1}>
                            {ward.name}
                            <Text as="span" fontSize="xs" color="gray.400" fontWeight="400" ml={1.5}>
                              ({ward.constituency.name})
                            </Text>
                          </Text>
                          <Text fontSize="xs" color="gray.400">
                            {ward.pollingStations.length} station{ward.pollingStations.length !== 1 ? "s" : ""}
                            {" · "}{wardStreams} stream{wardStreams !== 1 ? "s" : ""}
                          </Text>
                          {/* Add station button */}
                          <HStack
                            as="button"
                            gap={1} px={2} py={1} borderRadius="md"
                            fontSize="10px" fontWeight="600" color="gray.400"
                            cursor="pointer" _hover={{ color: "#0891b2", bg: "#e0f2fe" }}
                            transition="all 0.15s"
                            onClick={() => {
                        ...
                        {isOpen && (
                          <Box>
                            ...
                            {isAddingStation && (
                              <Box px={8} py={3} bg="#f0f9ff" borderTopWidth="1px" borderTopColor="#e0f2fe">
                                <Text fontSize="xs" fontWeight="600" color="#0369a1" mb={2}>
                                  Add Polling Station to <strong>{ward.name}</strong>
                                </Text>
                                <InlineForm
                                  fields={[
                                    { key: "name", label: "Name", placeholder: "Station name", required: true },
                                    { key: "code", label: "Code", placeholder: "Code e.g. PS-001", required: true },
                                    { key: "voters", label: "Voters", placeholder: "Registered voters (optional)", type: "number" },
                                  ]}
                                  onSave={(vals) => handleAddStation(ward.id, vals)}
                                  onCancel={() => { setAddingStationToWard(null); setAddStationError("") }}
                            ...
                            {/* Polling stations */}
                            {ward.pollingStations.length === 0 && !isAddingStation && (
                              <HStack px={8} py={3} gap={2} borderTopWidth="1px" borderTopColor="gray.50">
                                <Text fontSize="xs" color="gray.300">No polling stations &mdash; click &quot;+ Station&quot; to add one.</Text>
                              </HStack>
                            )}
                            ...
                            {ward.pollingStations
                              .sort((a, b) => a.name.localeCompare(b.name))
                              .map((ps, psIdx) => {
                                const isAddingStreamHere = addingStreamToStation === ps.id
                                return (
                                  <Box
                                    key={ps.id}
                                    borderTopWidth="1px"
                                    borderTopColor="gray.50"
                                    bg={psIdx % 2 === 0 ? "white" : "#fafafa"}
                                  >
                                    {/* Station row */}
                                    <HStack px={8} py={2.5} gap={3}>
                                      <FiMapPin fontSize="0.75rem" color="#9ca3af" />
                                      <Text fontSize="sm" color="gray.700" flex={1}>
                                        {ps.name}
                                        <Text as="span" fontSize="xs" color="gray.400" ml={1.5}>
                                          {ps.code}
                                        </Text>
                                      </Text>
                                      ...
                                      <HStack gap={1.5} flexWrap="wrap" flex={1}>
                                        {ps.streams.length === 0 && !isAddingStreamHere && (
                                          <Text fontSize="10px" color="gray.300">no streams</Text>
                                        )}
                                        {ps.streams.map((stream) => (
                                          <HStack key={stream.id} gap={0.5}>
                                            <Badge
                                              px={1.5} py={0.5} bg="#f3f4f6" color="gray.600"
                                              borderRadius="sm" fontSize="10px" fontWeight="600"
                                            >
                                              <HStack gap={1}>
                                                <FiGrid fontSize="0.55rem" />
                                                <Text>{stream.name || stream.code}</Text>
                                              </HStack>
                                            </Badge>
                                            <DeleteButton
                                              onConfirm={() => handleDeleteStream(ward.id, ps.id, stream.id)}
                                              isPending={isPending}
                                            />
                                          </HStack>
                                        ))}
                                      ...
                                      {ps.registeredVoters && (
                                        <Text fontSize="xs" color="gray.400" flexShrink={0}>
                                          {ps.registeredVoters.toLocaleString()} voters
                                        </Text>
                                      )}
                                      ...
                                      <DeleteButton
                                        onConfirm={() => handleDeleteStation(ward.id, ps.id)}
                                        isPending={isPending}
                                      />
                                    </HStack>
                                    ...
                                    {isAddingStreamHere && (
                                      <Box px={10} py={3} bg="#f0fdf4" borderTopWidth="1px" borderTopColor="#dcfce7">
                                        <Text fontSize="xs" fontWeight="600" color="#16a34a" mb={2}>
                                          Add Stream to <strong>{ps.name}</strong>
                                        </Text>
                                        <InlineForm
                                          fields={[
                                            { key: "name", label: "Name", placeholder: "e.g. Stream A", required: true },
                                            { key: "code", label: "Code", placeholder: "e.g. A", required: true },
                                            { key: "voters", label: "Voters", placeholder: "Registered voters (optional)", type: "number" },
                                          ]}
                                          onSave={(vals) => handleAddStream(ward.id, ps.id, vals)}
                                          onCancel={() => { setAddingStreamToStation(null); setAddStreamError("") }}
