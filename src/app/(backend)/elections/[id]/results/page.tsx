...
export default async function ElectionResultsPage({
  ...
  const sortedResults = [...results].sort(
    (a, b) => POSITION_ORDER.indexOf(a.positionType) - POSITION_ORDER.indexOf(b.positionType)
  )

  const totalStreamsReported = results.reduce((s, r) => s + r.streamStats.totalReported, 0)
  const totalVotesAll = results.reduce((s, r) => s + r.streamStats.totalVotes, 0)
    ...
    <VStack gap={6} alignItems="stretch" w="full">
        ...
        <HStack justify="space-between" align="flex-end" w="full" flexWrap="wrap" gap={3}>
          <VStack alignItems="flex-start" gap={0.5}>
            <HStack gap={2}>
              ...
              <Box px={2} py={0.5} bg="#eff9d1" color="#798217" borderRadius="md" fontSize="sm" fontWeight="700">
                {election.year}
              </Box>
            </HStack>
            <Text fontSize="sm" color="gray.500">{election.title}</Text>
          </VStack>
        </HStack>
      </VStack>
      ...
      <SimpleGrid columns={{ base: 2, md: 4 }} gap={4}>
        {[
          ...
          { label: "Validated",    value: results.reduce((s, r) => s + r.levelValidations, 0) },
        ].map(({ label, value }) => (
          <Box key={label} bg="white" p={4} borderRadius="xl" borderWidth="1px" borderColor="gray.100"
            boxShadow="0 1px 3px 0 rgba(0,0,0,0.06)">
            <Text fontSize="xs" color="gray.500" fontWeight="600" textTransform="uppercase" letterSpacing="wide" mb={1}>
              {label}
            </Text>
            <Text fontSize="2xl" fontWeight="800" color="gray.900" lineHeight="1.1">{value}</Text>
          </Box>
        ))}
      </SimpleGrid>
      ...
      {totalStreamsReported === 0 && (
        <Box bg="white" borderRadius="xl" p={12} textAlign="center" borderWidth="2px" borderStyle="dashed" borderColor="gray.200">
      ...
      {sortedResults.map((position) => {
        ...
        return (
          <Box key={position.positionId} bg="white" borderRadius="2xl" borderWidth="1px" borderColor="gray.100"
            ...
            <Box px={6} py={4} borderBottomWidth="1px" borderBottomColor="gray.100">
              <HStack justify="space-between" flexWrap="wrap" gap={3}>
                <HStack gap={3}>
                  <Flex w={9} h={9} borderRadius="lg" bg={lc.bg} align="center" justify="center" flexShrink={0}>
                    <MdHowToVote fontSize="1rem" color={lc.color} />
                  </Flex>
                  <VStack alignItems="flex-start" gap={0}>
                    <Text fontWeight="700" fontSize="sm" color="gray.900">{position.positionTitle}</Text>
                    <Text fontSize="xs" color="gray.500">
                      {position.streamStats.totalReported.toLocaleString()} streams ·{" "}
                      {position.streamStats.totalVotes.toLocaleString()} votes ·{" "}
                      {position.entities.length} {unitLabel.toLowerCase()}{position.entities.length !== 1 ? "s" : ""}
                    </Text>
                  </VStack>
                </HStack>
                <HStack gap={2}>
                  <Badge px={2.5} py={1} borderRadius="full" bg={lc.bg} color={lc.color}
                  ...
                  {position.levelValidations > 0 && (
                    <HStack gap={1}>
                      ...
                      <Text fontSize="xs" color="#10b981" fontWeight="600">{position.levelValidations} validated</Text>
                    </HStack>
                  )}
                  {!hasVotes && (
                    <HStack gap={1}>
            ...
            <VStack gap={0} alignItems="stretch" divideY="1px">
              {position.entities.map((entity) => {
                ...
                return (
                  <Box key={entity.entityId}>
                    {/* Entity label row */}
                    <Box px={6} py={2.5} bg="gray.50" borderBottomWidth="1px" borderBottomColor="gray.100">
                      <HStack justify="space-between">
                        <HStack gap={2}>
                          <Box w={1.5} h={4} bg={lc.border} borderRadius="full" />
                        ...
                        return (
                          <Box key={candidate.id}
                            ...
                            <HStack gap={4} align="center">
                              {/* Rank */}
                              <Flex w={6} h={6} borderRadius="full" flexShrink={0} align="center" justify="center"
                              ...
                              <Box flex={1} minW={0}>
                                <HStack justify="space-between" mb={1}>
                                  <HStack gap={2} minW={0}>
                                    ...
                                    {candidate.party && (
                                      <Text fontSize="xs" color="gray.400" flexShrink={0}>{candidate.party}</Text>
                                    )}
                                  </HStack>
              ...
              {position.entities.length === 0 && (
                <Box px={6} py={5} textAlign="center">
                  <Text fontSize="sm" color="gray.400">No candidates registered for this position.</Text>
                </Box>
              )}
            </VStack>
