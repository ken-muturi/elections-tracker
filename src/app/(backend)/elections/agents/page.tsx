      ...
      {Object.keys(agentData).length === 0 ? (
        ...
        >
          ...
          <Text fontSize="sm" color="gray.400">
            Assign agents to streams within each election to enable results data
            entry.
          </Text>
        </Box>
      ) : (
        <VStack gap={4} alignItems="stretch">
          {Object.values(agentData).map(({ election, assignments }) => {
            ...
            const agentsInElection = new Map<
              string,
              { name: string; email: string; streamCount: number }
            >();
            for (const a of assignments) {
              ...
              >
                ...
                >
                  ...
                  <Text fontSize="sm" color="gray.500" fontWeight="600">
                    ...
                    {agentsInElection.size} agent
                    {agentsInElection.size !== 1 ? "s" : ""}
                  </Text>
                </HStack>

                {/* Agents in this election */}
                <VStack px={5} py={4} gap={2} alignItems="stretch">
                  {Array.from(agentsInElection.entries()).map(
                    ([agentId, agent]) => (
                      <HStack
                        key={agentId}
                        gap={3}
                      ...
                      >
                        ...
                        <Badge
                          ...
                          fontWeight="600"
                        >
                          {agent.streamCount} stream
                          {agent.streamCount !== 1 ? "s" : ""}
                        </Badge>
                      </HStack>
                    ),
                  )}
